...
<<<<<<< HEAD
    ...
    public SendPayrollDialog(Window owner, boolean accountantMode) {
        ...
        filter.add(new JLabel("Tìm NV:")); filter.add(search);
        filter.add(new JLabel("Năm:")); filter.add(year);
        JButton find=Ui.btn("Tìm kiếm"); JButton send=Ui.btn("Gửi phiếu đã chọn"); JButton close=Ui.btn("Đóng");
        filter.add(find); filter.add(send); filter.add(close);
        add(filter,BorderLayout.SOUTH);
        add(new JScrollPane(table),BorderLayout.CENTER);

        Runnable load=this::load;
        find.addActionListener(e->load.run()); search.addActionListener(e->load.run()); year.addActionListener(e->load.run());
        send.addActionListener(e->send()); close.addActionListener(e->dispose()); load.run();
    }
    ...
    private void load(){
        try{
            ...
            for(Payroll p:service.hr(yearValue())){
                if(!"DRAFT".equals(p.status())) continue;
                if(!q.isEmpty() && !p.employeeCode().toLowerCase().contains(q) && !p.employeeName().toLowerCase().contains(q)) continue;
    ...
    private void send(){
        ...
        if(r<0){JOptionPane.showMessageDialog(this,"Hãy chọn phiếu DRAFT cần gửi.");return;}
        String id=table.getValueAt(r,0).toString(), code=table.getValueAt(r,1).toString(), name=table.getValueAt(r,2).toString();
        String period=table.getValueAt(r,3)+"/"+table.getValueAt(r,4);
        int ok=JOptionPane.showConfirmDialog(this,"Gửi phiếu lương kỳ "+period+" của "+code+" - "+name+" cho tài khoản nhân viên?\nSau khi gửi, nhân viên sẽ nhìn thấy phiếu trong 'Phiếu lương của tôi'.","Xác nhận gửi phiếu",JOptionPane.YES_NO_OPTION,JOptionPane.QUESTION_MESSAGE);
        if(ok!=JOptionPane.YES_OPTION)return;
        try{service.send(Integer.parseInt(id),Integer.parseInt(code.replace("NV","")));load();JOptionPane.showMessageDialog(this,"Đã gửi phiếu lương cho "+code+".");}catch(Exception e){Ui.error(this,e);}
    }
