package com.hrm.gui;
<<<<<<< HEAD
import com.hrm.service.*;import javax.swing.*;import java.awt.*;import java.time.LocalDate;
public class HRHistoryPanel extends JPanel{
 ...
 public HRHistoryPanel(){this(true);}
 public HRHistoryPanel(boolean editable){
  setLayout(new BorderLayout(8,8));add(Ui.top(editable?"Sự kiện nhân sự - thưởng, phạt, nghỉ, thăng chức":"Sự kiện nhân sự - chế độ xem của Kế toán trưởng"),BorderLayout.NORTH);
  JPanel b=new JPanel(new FlowLayout(FlowLayout.LEFT));b.add(new JLabel("Tìm NV:"));b.add(q);b.add(new JLabel("Năm:"));b.add(y);if(editable)b.add(penalty);add(b,BorderLayout.SOUTH);add(new JScrollPane(t),BorderLayout.CENTER);
  Runnable load=()->{try{Ui.fill(t,new HRHistoryService().find(yearValue(),q.getText().trim()));}catch(Exception e){Ui.error(this,e);}};
  y.addActionListener(e->load.run());q.addActionListener(e->load.run());if(editable)penalty.addActionListener(e->showPenalty());load.run();
 }
 private int yearValue(){String v=(String)y.getSelectedItem();return "Tất cả 5 năm".equals(v)?0:Integer.parseInt(v);}
 private void showPenalty(){JPanel f=new JPanel(new GridLayout(0,2,8,8));JTextField code=new JTextField(),amount=new JTextField("500000"),reason=new JTextField("Nghỉ quá số ngày quy định");f.add(new JLabel("Mã NV:"));f.add(code);f.add(new JLabel("Số tiền trừ:"));f.add(amount);f.add(new JLabel("Lý do:"));f.add(reason);f.add(new JLabel("Ngày:"));f.add(new JTextField(LocalDate.now().toString()));int ok=JOptionPane.showConfirmDialog(this,f,"Ghi phạt / trừ lương",JOptionPane.OK_CANCEL_OPTION);if(ok!=JOptionPane.OK_OPTION)return;try{int id=Integer.parseInt(code.getText().trim().toUpperCase().replace("NV",""));double a=Double.parseDouble(amount.getText().trim());String rs=reason.getText().trim();new DisciplineService().add(id,a,rs,LocalDate.now());JOptionPane.showMessageDialog(this,"Đã ghi khoản phạt. Khi tạo phiếu lương, khoản này sẽ được cộng vào Khấu trừ.");}catch(Exception e){Ui.error(this,e);}}
=======
