...
<<<<<<< HEAD
    private final JTable t=Ui.table(new String[]{"Mã NV","Nhân viên","Tháng","Năm","Lương CB","OT","Thưởng","Phạt / trừ lương","Thực lĩnh","Trạng thái"});
    ...
    public MyPayslipPanel(Integer emp){
        this.emp=emp; setLayout(new BorderLayout(8,8));
        add(Ui.top("Phiếu lương của tôi - chỉ phiếu HR đã gửi"),BorderLayout.NORTH);
        JPanel b=new JPanel(new FlowLayout(FlowLayout.LEFT)); b.add(new JLabel("Năm:")); b.add(y);
        JButton detail=Ui.btn("Xem phiếu lương chi tiết"); b.add(detail);
        add(b,BorderLayout.SOUTH); add(new JScrollPane(t),BorderLayout.CENTER);
        y.addActionListener(e->load()); detail.addActionListener(e->showDetail());
    ...
    private void load(){try{var rows=new ArrayList<String[]>();for(Payroll p:new PayrollService().mine(emp,yearValue()))rows.add(new String[]{p.employeeCode(),p.employeeName(),String.valueOf(p.month()),String.valueOf(p.year()),fmt(p.base()),fmt(p.overtime()),fmt(p.bonus()),fmt(p.deduction()),fmt(p.net()),p.status()});Ui.fill(t,rows);}catch(Exception e){Ui.error(this,e);}}
    private void showDetail(){int r=t.getSelectedRow();if(r<0){JOptionPane.showMessageDialog(this,"Chọn một phiếu lương trước.");return;}int month=Integer.parseInt(t.getValueAt(r,2).toString()),year=Integer.parseInt(t.getValueAt(r,3).toString());try{for(Payroll p:new PayrollService().mine(emp,year))if(p.month()==month&&p.year()==year){new PayslipDetailDialog(SwingUtilities.getWindowAncestor(this),p).setVisible(true);return;}}catch(Exception e){Ui.error(this,e);}}
