...
=======
        ...
        JLabel title = new JLabel("PHIẾU LƯƠNG CHI TIẾT");
        title.setFont(title.getFont().deriveFont(Font.BOLD, 22f));
        JLabel info = new JLabel(p.employeeCode() + " - " + p.employeeName() + "   |   Kỳ lương: " + p.month() + "/"
                + p.year() + "   |   " + p.status());
        info.setFont(info.getFont().deriveFont(Font.PLAIN, 14f));
        ...
        try {
            ...
            double insuranceSalary = p.base();
            double insurance = PayrollCalculator.insurance(insuranceSalary);
            double personal = p.year() >= 2026 ? PayrollCalculator.PERSONAL_DEDUCTION_2026
                    : PayrollCalculator.PERSONAL_DEDUCTION_OLD;
            double dependentUnit = p.year() >= 2026 ? PayrollCalculator.DEPENDENT_DEDUCTION_2026
                    : PayrollCalculator.DEPENDENT_DEDUCTION_OLD;
...
=======
                    ...
                    { "Giảm trừ bảo hiểm bắt buộc (10,5%)", money(insurance) },
                    { "Giảm trừ bản thân", money(personal) },
                    { "Giảm trừ NPT (" + dependents + " người × " + String.format("%,.0f", dependentUnit) + ")",
                            money(dependent) },
                    { "Các khoản không chịu thuế", "0" },
                    { "Tổng giảm trừ", money(totalDeduction) },
                    { "THU NHẬP TÍNH THUẾ", money(taxable) },
            ...
            JLabel note = new JLabel(
                    "BH bắt buộc = 10,5% lương đóng BHXH; PIT tính theo biểu thuế lũy tiến của từng kỳ (2026 dùng 5 bậc).");
            note.setBorder(BorderFactory.createEmptyBorder(4, 12, 4, 12));
