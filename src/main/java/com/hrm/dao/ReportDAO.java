...
public class ReportDAO {
    public List<String[]> staffing() throws SQLException {
        ...
        try(Connection c=DBConnection.getConnection();PreparedStatement p=c.prepareStatement(q);ResultSet r=p.executeQuery()){
            while(r.next()) a.add(new String[]{r.getString(1),r.getString(2),r.getString(3),r.getString(4),r.getString(5)});
        } return a;
    }
    public List<String[]> projects() throws SQLException {
        ...
        try(Connection c=DBConnection.getConnection();PreparedStatement p=c.prepareStatement(q);ResultSet r=p.executeQuery()){
            while(r.next()) a.add(new String[]{r.getString(2),r.getString(3),r.getString(4),r.getString(5),r.getString(6),r.getString(7),r.getString(8)});
        } return a;
    }
    public List<String[]> employeesByJob(String job,String text) throws SQLException {
        ...
        try(Connection c=DBConnection.getConnection();PreparedStatement ps=c.prepareStatement(q)){ps.setString(1,job);ps.setString(2,text);ps.setString(3,"%"+text+"%");ps.setString(4,"%"+text+"%");try(ResultSet r=ps.executeQuery()){while(r.next())a.add(new String[]{r.getString(1),r.getString(2),r.getString(3),r.getString(4),r.getString(5),r.getString(6)});}}
        return a;
    }
    public List<String[]> employeesByJob(String job) throws SQLException{return employeesByJob(job,"");}
    public List<String[]> employeesByProject(String project,String text) throws SQLException {
        ...
        try(Connection c=DBConnection.getConnection();PreparedStatement ps=c.prepareStatement(q)){ps.setString(1,project);ps.setString(2,text);ps.setString(3,"%"+text+"%");ps.setString(4,"%"+text+"%");try(ResultSet r=ps.executeQuery()){while(r.next())a.add(new String[]{r.getString(1),r.getString(2),r.getString(3),r.getString(4),r.getString(5),r.getString(6)});}}
        return a;
    }
    public List<String[]> employeesByProject(String project) throws SQLException{return employeesByProject(project,"");}
    public double totalPaid(int year)throws SQLException{String q="SELECT COALESCE(SUM(net_salary),0) FROM payrolls WHERE status='SENT' AND (?=0 OR pay_year=?)";try(Connection c=DBConnection.getConnection();PreparedStatement p=c.prepareStatement(q)){p.setInt(1,year);p.setInt(2,year);try(ResultSet r=p.executeQuery()){r.next();return r.getDouble(1);}}}
    public int count(String table)throws SQLException{if(!table.matches("employees|departments|projects|payrolls"))throw new IllegalArgumentException();try(Connection c=DBConnection.getConnection();PreparedStatement p=c.prepareStatement("SELECT COUNT(*) FROM "+table);ResultSet r=p.executeQuery()){r.next();return r.getInt(1);}}
}
